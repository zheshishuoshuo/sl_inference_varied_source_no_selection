...
def run_mcmc(data_df,
    ...
    from likelihood import initializer_for_pool
...
with multiprocessing.get_context("spawn").Pool(
    ...
    initargs=(data_df, interp_list[0], interp_list[1], True)
) as pool:
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_posterior_func, pool=pool, backend=backend
    )
    sampler.run_mcmc(p0, nsteps, progress=True)
