...
# ==============================
# 生成 logRe（给定 logM_star）
# ==============================
def logRe_given_logM(logM_star, model='deVauc', random_state=None):
    p = MODEL_PARAMS[model]
    mu_Re = p['mu_R0'] + p['beta_R'] * (logM_star - 11.4)
    return norm.rvs(loc=mu_Re, scale=p['sigma_R'], size=len(logM_star), random_state=random_state)

# ==============================
# 生成 logMh（给定 logM_star 和 logRe）
# ==============================
def logMh_given_logM_logRe(logM_star, logRe, model='deVauc', random_state=None):
    p = MODEL_PARAMS[model]
    mu_r = p['mu_R0'] + p['beta_R'] * (logM_star - 11.4)
    mu_h = p['mu_h0'] + p['beta_h'] * (logM_star - 11.4) + p['xi_h'] * (logRe - mu_r)
    return norm.rvs(loc=mu_h, scale=p['sigma_h'], size=len(logM_star), random_state=random_state)
...
def generate_samples(n_samples, model='deVauc', random_state=None):
    ...
    """
    logM_star = mstar_gene(n_samples, model=model, random_state=random_state)
    logRe = logRe_given_logM(logM_star, model=model, random_state=random_state)
    logMh = logMh_given_logM_logRe(logM_star, logRe, model=model, random_state=random_state)
