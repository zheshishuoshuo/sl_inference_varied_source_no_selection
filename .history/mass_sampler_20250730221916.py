...
if __name__ == '__main__':
    ...
    axes[0].hist(samples['logM_star'], bins=50, density=True, alpha=0.6, color='orange', label='Sampled $\\log M_*$')
    x_Mh = np.linspace(10.5, 12.5, 500)
    
    params = MODEL_PARAMS['deVauc']
    pdf = skewnorm.pdf(x, a=10**params['log_s_star'], loc=params['mu_star'], scale=params['sigma_star'])
    axes[0].plot(x, pdf, 'k--', lw=2, label='Sonnenfeld+19 PDF')
    axes[0].set_xlabel('$\\log M_*$')
    axes[0].set_ylabel('PDF')
    axes[0].legend()
    axes[0].set_title('Distribution of $\\log M_*$')    
    axes[1].hist(samples['logRe'], bins=50, density=True, alpha=0.6, color='blue', label='Sampled $\\log R_e$') 
    x_Re = np.linspace(0.5, 2.5, 500)
    mu_Re = MODEL_PARAMS['deVauc']['mu_R0'] + MODEL_PARAMS['deVauc']['beta_R'] * (samples['logM_star'] - 11.4)
    pdf_Re = norm.pdf(x_Re, loc=mu_Re.mean(), scale=MODEL_PARAMS['deVauc']['sigma_R'])
    # === Re 分布 ===
    mu_Re = MODEL_PARAMS['deVauc']['mu_R0'] + MODEL_PARAMS['deVauc']['beta_R'] * (samples['logM_star'] - 11.4)
    pdf_Re = np.mean([
        norm.pdf(x_Re, loc=mu, scale=MODEL_PARAMS['deVauc']['sigma_R'])
        for mu in mu_Re
    ], axis=0)

    axes[1].hist(samples['logRe'], bins=50, density=True, alpha=0.6, color='blue', label='Sampled $\\log R_e$') 
    axes[1].plot(x_Re, pdf_Re, 'k--', lw=2, label='Model PDF for $\\log R_e$')

    # === Mh 分布 ===
    mu_Mh = (MODEL_PARAMS['deVauc']['mu_h0']
            + MODEL_PARAMS['deVauc']['beta_h'] * (samples['logM_star'] - 11.4)
            + MODEL_PARAMS['deVauc']['xi_h'] * (samples['logRe'] - mu_Re))
    pdf_Mh = np.mean([
        norm.pdf(x_Mh, loc=mu, scale=MODEL_PARAMS['deVauc']['sigma_h'])
        for mu in mu_Mh
    ], axis=0)

    axes[2].hist(samples['logMh'], bins=50, density=True, alpha=0.6, color='green', label='Sampled $\\log M_h$')    
    axes[2].plot(x_Mh, pdf_Mh, 'k--', lw=2, label='Model PDF for $\\log M_h$')
