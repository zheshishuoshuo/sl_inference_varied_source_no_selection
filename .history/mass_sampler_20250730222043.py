...
# 假设 generate_samples 和 MODEL_PARAMS 已定义
# from your_module import generate_samples, MODEL_PARAMS
...
axes[0].plot(x_logM, pdf_logM, 'k--', lw=2, label='Model PDF')
...
mu_Re_array = params['mu_R0'] + params['beta_R'] * (samples['logM_star'] - 11.4)
pdf_logRe = np.mean([
    norm.pdf(x_Re, loc=mu, scale=params['sigma_R']) for mu in mu_Re_array
], axis=0)

axes[1].hist(samples['logRe'], bins=50, density=True, alpha=0.6, color='blue', label='Sampled $\\log R_e$')
axes[1].plot(x_Re, pdf_logRe, 'k--', lw=2, label='Model PDF')
...
# 3. 绘制 logMh 分布（混合高斯）
# --------------------------------------------------
x_Mh = np.linspace(11.0, 15.0, 500)
mu_Mh_array = (
    params['mu_h0']
    + params['beta_h'] * (samples['logM_star'] - 11.4)
    + params['xi_h'] * (samples['logRe'] - mu_Re_array)
)
pdf_logMh = np.mean([
    norm.pdf(x_Mh, loc=mu, scale=params['sigma_h']) for mu in mu_Mh_array
], axis=0)

axes[2].hist(samples['logMh'], bins=50, density=True, alpha=0.6, color='green', label='Sampled $\\log M_h$')
axes[2].plot(x_Mh, pdf_logMh, 'k--', lw=2, label='Model PDF')
